# -*- coding: utf-8 -*-
"""
Created on Thu Oct 27 14:09:09 2016
...
branch_ngram_2.add(Convolution2D(100, 2, ACTION_MAX_LENGHT, input_shape=(2,ACTIVITY_MAX_LENGHT,ACTION_MAX_LENGHT),border_mode='valid',activation='relu'))
branch_ngram_2.add(MaxPooling2D(pool_size=(ACTIVITY_MAX_LENGHT-2+1,1))) # 1-max-pool

branch_ngram_3= Sequential()
branch_ngram_3.add(Convolution2D(100, 3, ACTION_MAX_LENGHT, input_shape=(2,ACTIVITY_MAX_LENGHT,ACTION_MAX_LENGHT),border_mode='valid',activation='relu'))
branch_ngram_3.add(MaxPooling2D(pool_size=(ACTIVITY_MAX_LENGHT-3+1,1))) # 1-max-pool

branch_ngram_4= Sequential()
branch_ngram_4.add(Convolution2D(100, 4, ACTION_MAX_LENGHT, input_shape=(2,ACTIVITY_MAX_LENGHT,ACTION_MAX_LENGHT),border_mode='valid',activation='relu'))
branch_ngram_4.add(MaxPooling2D(pool_size=(ACTIVITY_MAX_LENGHT-4+1,1))) # 1-max-pool

merged = Merge([branch_ngram_2, branch_ngram_3, branch_ngram_4], mode='concat', concat_axis=2)
...
for i, activity in enumerate(activities):
    ...
    activity_actions = np.array([actions, previous_actions])
...
X = X.reshape(X.shape[0], 2, ACTIVITY_MAX_LENGHT, ACTION_MAX_LENGHT)
X_test = np.array(X_test)
y_test = np.array(y_test)
print 'Activity distribution for testing:'
check_activity_distribution(y_test, unique_activities)
X_test = X_test.reshape(X_test.shape[0], 2, ACTIVITY_MAX_LENGHT, ACTION_MAX_LENGHT)
print 'Shape (X,y):'
print X.shape
print y.shape
...
print 'Training...'
sys.stdout.flush()
classifier.fit([X, X, X], y, batch_size=10, nb_epoch=3000, validation_data=([X_test, X_test, X_test], y_test))
print 'Saving model...'
sys.stdout.flush()
